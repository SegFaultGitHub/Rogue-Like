...
public class RuleTileGenerator : EditorWindow {
    ...
    private void OnGUI() {
        ...
        if (templ_neighbors.Count > 0) {
            ...
            if (templateSprites.Length != tileSprites.Length)
                ...
                DisplayTilemapPreview(collumns, tileSprites);
            ...
            if (tileSprites.Length == 0)
                GUILayout.Box("Set sprites to show other options", EditorStyles.helpBox);
            else if (tileSprites.Length != templateSprites.Length)
                GUILayout.Box("Amount of sprites needs to be the same as the template", EditorStyles.helpBox);
            else {
                if (setDefaultIndex) {
    ...
    public RuleTile GenerateRuleTile() {
        ...
        for (int i = 0; i < tileSprites.Length; i++) {
            ...
            if (addGameobjectsToRules)
                rule.m_GameObject = defaultGameobject;
